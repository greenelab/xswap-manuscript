...
To begin, we chose the STRING PPI network for the comparison, and computed five edge prediction features (Supplemental table {@tbl:edge-prediction}) on the network.
...
![Common edge-prediction features are correlated with node degree.
  Shown are five features (Supplemental table {@tbl:edge-prediction}) computed on STRING PPI network [@doi:10.1093/nar/gky1131] and the product of the node pair's source and target degrees.
...
The network reconstruction performance of each feature was compared with two permutation-derived null-values.
First, the performance of the edge prior was compared to determine the feature performance fraction attributable to the degree sequence of the PPI network.
...
Each permutation corresponds to an AUROC value quantifying the performance of features computed on it.
The second comparison gave insight into the performance of each feature if it was only picking up on degree.
...
Degree was able to reconstruct the PPI network with an AUROC of 0.797.
This result indicated that a nonspecific predictions alone had this level of performance.
The second comparison showed that four out of five edge prediction features pick up on more than node degree for predictions.
