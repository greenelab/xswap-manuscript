...
The algorithm picks two existing edges at random, and if the edges constitute a valid swap, exchanges the targets between the edges (Supplemental Table {@tbl:xswap}).
...
](images/algorithms_label.png){#fig:algo width="85%"}
...
The edge prior is a numerical value that can be computed for every pair of nodes that could potentially share an edge, and we compared its ability to predict edges in three tasks, discussed below.
...
We compared the permutation-based prior with two additional predictors: our analytical approximation of the edge prior and the product of source and target degree, scaled to the range [0, 1] to allow calibration assessment.
...
We used this to augment each node pair's predictor values in permuted networks, which allowed these pairs to have more permuted predictor values than permuted networks.
...
The full source code has also been made freely available under the BSD 2-Clause License (<https://github.com/hetio/xswap>).
...
![
  **Edge prior shows strong performance for unsampled and sampled network reconstruction**
  The edge prior shows strong performance in reconstructing the network on which it was computed (Task 1, 20 different networks) and the original network when computed on only a sample of the edges (Task 2, 20 different networks).
  The edge prior shows much lower performance when computed on a completely different network of the same type of data (Task 3, 3 different networks).
...
The three predictors that we compared were highly rank correlated (over 0.984 for all 20 networks).
...
Our permutation framework meatheysures the predictive performance attributable to degree to provide a baseline expectation for edge pairs.
