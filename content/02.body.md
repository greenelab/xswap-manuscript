...
Degree is an important metric for differentiating between nodes and it is heavily relied-upon by a number of edge prediction (also known as "link prediction") features (Figure {@fig:degree-bias}E).
...
Depending on the methods by which the represented data were generated, networks of the same type of data may assign very different degree to the same nodes (Figure {@fig:degree-bias}B-C), and they may have overall degree distributions that differ greatly (Figure {@fig:degree-bias}A).
...
Shown are five features (Supplemental table {@tbl:edge-prediction}) computed on literature-derived protein-protein interaction network [@doi:10.1093/nar/gky1131] and the geometric mean of the node pair's source and target degrees.
All five features show a positive relationship with degree, though the magnitude of this correlation is highly variable.
](https://github.com/zietzm/xswap-analysis/raw/new-plots/img/fig1.bias_features_combined.png){#fig:degree-bias width="100%"}
