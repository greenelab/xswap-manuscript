...
This phenomenon of edges missing in a non-uniform manner can be thought-of as a kind of sampling bias.

Bias in the structure of data networks can be demonstrated in domains where unbiased experimentation methods exist, for example high-throughput, hypothesis-free experimentation.
...
If the biases of high-throughput experimentation are roughly degree-independent, then the degree distributions of unbiased networks resemble the degree distributions of true relationships.

Figure {@fig:node-degree-bias} shows the phenomenon of node degree bias in six networks, two protein-protein interaction networks, one of which is literature-derived, and the other of which was constructed via a high-throughput screen of all human proteins, and two transcription factor-target gene networks, one of which is literature-derived, and the other of which was constructed from a ChIP-seq screen, and a co-authorship network based on bioinformatics preprints on bioRxiv.

![Degree distributions of biased and unbiased networks are very different, though not consistently. Protein-protein interaction (PPI) literature-derived network from STRING (Szklarczyk et al.) [@doi:10.1093/nar/gky1131], high-throughput-derived networks from Rual et al. [@doi:10.1038/nature04209] and Rolland et al. [@doi:10.1016/j.cell.2014.10.050]. Transcription factor-target gene (TFTG) literature-derived network from Han et al. [@doi:10.1093/nar/gkx1013] and high-throughput network from Lachmann et al. [@doi:10.1093/bioinformatics/btq466].  BioRxiv data from Rxivist [@doi:10.1101/515643; @doi:10.5281/zenodo.2566421]. `Should have "literature-derived", and "high-throughput" or "hypothesis-free" rather than "biased" and "unbiased".`](images/1_degree_bias/degree_dists.png){#fig:node-degree-bias width="100%}

Networks created through unbiased experimentation have degree distributions that are similar to the distribution of degrees for true relationships.
...
![Common link-prediction features are correlated with node degree. Shown are five features computed on literature-derived protein-protein interaction network [@doi:10.1093/nar/gky1131] and the geometric mean of the node pair's source and target degrees. All five features show a positive relationship with degree, though the magnitude of this correlation is highly variable. For example, preferential attachment and random walk with restart (RWR) appear to have stronger relationships with degree than the Jaccard coefficient. `Should make headers formatted strings, not including underscores.`](images/2_link_prediction_feature_degree_correlation/02.feature_degree_correlation.png){#fig:features-degree width="100%"}
...
For example, the resource allocation index is the sum of inverse degree of common neighbors between source and target nodes (in the symmetric case) [@doi:10.1140/epjb/e2009-00335-8].
...
In addition to being influenced by degree, the values of a single feature for various network entities (whether nodes, node pairs, communities, etc.), computed for the purposes of analysis or prediction are not, in general, meaningfully comparable to one another.
The raw values of features computed for a given network entity depend both on relevant and non-relevant information, including extraneous structural properties of the network.
...
For this reason, the strength of signal for an entity from a feature should be obtained not by comparing entities to other entities, but by comparing feature values for a single entity with the portion of its value attributable solely to extraneous structural information.

Because degree is not, in general, reflective of the relative true degree of an entity, and because degree is highly influential in many features used for link prediction, we believe that degree is a confounding variable in biomedical link prediction.
...
We used three features for the unsupervised edge prediction task: random walk with restart (RWR), the Jaccard coefficient, and an inference score developed for link prediction in directed graphs [@raw:inference]. 
`Add a discussion of the features here. Why they were chosen, etc.`
...
![`This is not the correct image. Need to redo the analysis as explained above.`](images/4_ROC/all_roc.png){#fig:ROC width="100%"}
...
![Comparison of degree-feature relationship between raw link prediction features and features corrected for degree using counterfactual distributions computed on permuted networks](images/5_corrected_features_not_correlated/feature_vs_corrected.png){#fig:feature-vs-corrected width="75%"}
...
![Percent of edges swapped vs number of attempts at edge swaps. "Density" means the number of edges by the number of node pairs.](images/6_xswap_percent_swapped_iterations/percent_swapped_continuous.png){#fig:swap-percent width="100%"}
