...
Similarly, we can see that the Adamic/Adar index picked up almost perfectly on degree because its performances from permuted networks are nearly at the performance of the edge prior.
However, the Adamic/Adar index had much higher performance when computed on the unpermuted network, indicating that it also picks up on higher-order information in addition to degree.
