...
Nodes in biomedical networks can have different degrees, and networks can have very different degree distributions.
Many edge prediction methods are correlated with degree.
Degree imbalance, in which edges are distributed unequally between nodes, can lead edge prediction methods to make nonspecific or trivial predictions.
Sometimes networks faithfully represent all true relationships, but degree biases and other misrepresentations can obscure node degree.
To better understand the effects of degree on an edge prediction task and on edge prediction methods, degree should be considered directly.
We developed a network permutation framework and open source software implementation that quantifies the probability of edge existence due to degree and can be used to assess the fraction of feature performance attributable to degree.
...
We provided an example application of our methods that assessed five edge prediction features' performances on a network reconstruction task by quantifying the methods' reliance on degree and degree's performance on the prediction task.
Our method provides a context for evaluating edge prediction methods and specific predictions for reliance on degree and, therefore, nonspecificity.
