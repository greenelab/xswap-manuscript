...
False positives and false negatives will be found in almost every type of biomedical network.
...
This particular kind of bias is likely when networks are constructed using hypothesis-driven findings extracted from the literature, as newly-discovered relationships are not randomly sampled from all true relationships.
Bias in the degree structure of networks can be demonstrated in domains where unbiased experimentation methods exist, for example by comparing node degree between literature-derived and unbiased networks.

Figure {@fig:node-degree-bias} gives examples of degree distribution bias in networks representing three types of relationships: protein-protein interaction (PPI), transcription factor-target gene (TF-TG), and bioRxiv preprint co-authorship.
...
On the one hand, when edges are uniformly sampled there remains a linear correlation between degree in the networks (Figure {@fig:sampling_vs_systematic}a).
...
Figure {@fig:sampling_vs_systematic}b shows an example of degree biases that can be thought-of as degree-biased sampling.
...
![Degree distributions of biased and unbiased networks can be very different.
  ...
  BioRxiv data from Rxivist [@doi:10.1101/515643; @doi:10.5281/zenodo.2566421] split between edges existing before and during/after 2018.](https://github.com/greenelab/xswap-analysis/raw/bd40eb244ffe37454dc08099f5742ef3e962c70d/img/1_degree_bias/histogram.png){#fig:node-degree-bias width="100%}

![Individual node degree by sampling method.
  In the left column, 70% of literature edges were sampled with uniform probability.
  In the right column, literature derived degrees are over or under the corresponding systematically-derived degree, and are not correlated as is the case in unbiased sampling.](https://github.com/greenelab/xswap-analysis/raw/bd40eb244ffe37454dc08099f5742ef3e962c70d/img/7_well_studied_degree_bias/hexbin.png){#fig:sampling_vs_systematic width="85%"}
...
The raw values of features computed for a given network entity depend both on relevant and non-relevant information, including extraneous and biased structural properties of the network, such as degree sequence or network density.
...
![XSwap algorithm applied to two edges.
The algorithm preserves both the source- and target-degree for all nodes, making it generalizable to directed, undirected, and bipartite networks.](images/xswap_figure.png){#fig:xswap width="50%"}
