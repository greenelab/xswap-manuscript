...
Edge prediction methods tend to be correlated with degree (Figure {@fig:feature-degree}A).
To assess the influence of degree on the performance of edge prediction features, we computed each feature on a number of permuted networks to find a distribution of performance values attributable only to degree.
As a further basis of comparison, we computed the performance of the edge prior
However, all features with the exception of the preferential attachment index unequivocally pick up on more than node degree for predictions.
...
](images/feature_degree_combined.png){#fig:feature-degree width="100%"}
