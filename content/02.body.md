...
The comparison of features demonstrated that some features (eg: RWR) captured more than degree for the prediction task, while others (preferential attachment index) did not.
RWR captured more than degree because it reached all of a node's neighbors in one step after a restart.
