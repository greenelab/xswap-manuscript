...
The comparison of features demonstrated that some features (eg: RWR) pick up on far more than degree for the prediction task, while others (preferential attachment index) do not.
RWR picked up on more than degree because it reached all of a node's neighbors in one step after a restart.
Naturally, the preferential attachment index did not pick up on more than degree because its value is simply the product of source and target degrees.
The edge prior provided the baseline level of performance that is attributable to degree alone.
Features whose performances on permuted networks were below that of the edge prior only imperfectly pick up on degree (eg: Jaccard index), whereas those with performances exceeding the edge prior pick up on more than degree.
The no-holdout reconstruction task we conducted was intentionally simple to demonstrate that the edge prior and permutation framework captures the effects of degree in a task where the results could be intuited.
