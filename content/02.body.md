...
However, the problem of nonspecific predictions related to node degree is an even more general issue than gene multifunctionality.

Real networks can have a wide variety of degree distributions, and degree imbalance is a common characteristic.
Figure {@fig:degree-bias}D shows examples of different degree distributions in six biomedical networks.
Large degree imbalance can lead especially high-degree nodes to dominate in predictions.
The result is that degree-based predictions are more likely nonspecific, meaning the same set of predictions would appear to perform well for different tasks.
