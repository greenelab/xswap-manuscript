...
Second, edge prediction methods relying heavily on degree may be nonspecific---predicting trivial rather than insightful new relationships.
...
While nonspecificity may be acceptable in some cases, we believe that specific predictions are desirable for a range of prediction tasks in biomedicine as they represent insight gained from the actual connections in a network.
...
![
  **Degree can predict edges within a given network but does not generalize to networks with different degree distributions**
  The edge prior is able to reconstruct the networks on which it was computed (Task 2, "unsampled", 20 different networks) with high performance.
