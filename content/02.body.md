...
Supervised network reconstruction tasks are useful for many methods, especially for evaluating and comparing methods.
...
Because each edge is equally likely to have been dropped, this method does not introduce new degree bias, and the resulting network has a degree distribution that is essentially equivalent to the source network (Supplemental figure {@fig:supp-dists}.
...
To compare features by their ability to predict edges in an unsupervised manner in each task's test network, we used ROC curves and their areas (AUROC).
...
RWR was implemented as defined in [@raw:rwr], with the steady-state distribution ($\mathbf{x}_i$) given as follows, where $c$ is the probability of a restart, $A$ is the normalized adjacency matrix, and $\mathbf{y}_i$ is a vector with zero at all entries except $i$, where it is equal to one:
...
Because only degree is preserved, nodes with equal degree can be grouped in summarizing features.
...
For each of the PPI and TF-TG network types there were three networks on which features were computed, a degree-biased network, a sampled version of the degree-biased network, in which a fraction of edges were dropped, and a systematic network, in which no degree bias is expected.
...
We used our network permutation algorithm to generate 1000 degree-preserving permutations of each of these networks, and we computed the same set of features on each permutation of each of the nine individual networks (three for each of PPI, TF-TG, co-authorship).
...
In particular, for each network type (PPI, TF-TG, co-authorship), we evaluated the ability of each sub-network's features to predict new edges in other networks.
...
![Comparison of AUROC values](https://github.com/greenelab/xswap-analysis/raw/ecaa25fd998e5f11d25e13894157f05d0cd2ebb6/img/4_ROC/area_bars.png){#fig:AUROC width="100%"}
...
We find consistently lower performance in predicting systematically-derived edges than reconstructing the original network (Figures {@fig:ROC}, {@fig:AUROC}).
Additionally, we find noticable differences between the relative performances of raw, permutation-derived, and corrected features between undirected and directed networks.
...
This result is unsurprising, as several link prediction features are computed as functions of degree (`Supplemental table 1`).
...
Data networks in any domain where relationship data are not generated through unbiased means will be subject, to some extent, to degree bias.
...
In heterogeneous networks, features can be computed on a path-type (or "metapath") basis, meaning that many types of degree bias could be included in every feature.
