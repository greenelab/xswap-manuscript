...
Network permutation eliminates true connections, but it can be customized to preserve certain properties.
Permuted networks can be used to evaluate the fraction of edge prediction performance that is attributable entirely to preserved properties.
A method which has strong reconstruction performance on a true network and very poor performance on permuted networks likely relies on information that is eliminated in permutation.
...
To determine the extent to which an edge prediction method relies on degree, we propose comparing the method on a true network and degree-preserving permutations of the network.
Network permutation is flexible and can be broadly applied because it generates networks with an identical format to the original network.
This means that any network-based method can be analyzed for reliance on degree by using degree-preserving network permutation.
