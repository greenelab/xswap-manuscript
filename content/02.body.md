...
The edge prior can be considered a baseline edge predictor that excellently captures degree's contribution to the probability of an edge existing.
...
In drug repurposing, for example, existing compound-disease treatment relationships are not randomly sampled from all true treatment relationships.
However, all treatment relationships between existing compounds and diseases are desirable outputs in prediction.
Framed as edge prediction in this way, drug repurposing attempts to predict between different degree distributions.
